  ...
  if (!!$('#menu-name').val()) {
    $.ajax({
      ...
      data: {
        ...
        locale_parent_id:$('#locale_parent_id').val(),
      },

      url: createnewmenur,
      type: 'POST',
      success: function(response) {
        window.location = menuwr + '?menu=' + response.resp+ '&locale_id=' + response.locale_id+ '&locale_parent_id=' + response.locale_parent_id;
      }
    });
  } else {
