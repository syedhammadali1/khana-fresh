...
$currentUrl = url()->current() . '/locale/redirect';
$request = request();
$lang_menus = '';
if ($request->menu) {
            ...
            if(  $menu){
                $lang_menus =  DB::table('admin_menus')->orwhere(function ($q) use ($menu) {
                    $q->where('id', ($menu->locale_parent_id)?$menu->locale_parent_id:$menu->id);
            })->orwhere(function ($q) use ($menu) {
                if ($menu->locale_parent_id !== null) {
                    $q->where('locale_parent_id', $menu->locale_parent_id);
                } else {
                            ...
                            <div class="manage-menus">
                                <form method="get" action="{{ $currentUrl }}">
                                    ...
                                    <span class="add-new-menu-action">
                                        or
                                        <a href="{{ $currentUrl }}?action=edit&menu=0">
                                            Create new menu
                                        </a>.
                                    </span>
                                            ...
                                            <div class="menu-edit ">
                                                <div id="nav-menu-header">
                                                    <div class="major-publishing-actions">
                                                        <label class="menu-name-label howto open-label" for="menu-name">
                                                            ...
                                                            <input type="hidden" id="locale_id"
                                                                value="{{ request()->locale_id }}" />
                                                            <input type="hidden" id="locale_parent_id"
                                                                value="{{ request()->locale_parent_id }}" />
                                                        </label>
                                                        ...
                                                        @if (request()->has('action'))
                                                            <div class="publishing-action">
                                                                <a onclick="createnewmenu()" name="save_menu"
                                                                    id="save_menu_header"
                                                                    class="btn btn-primary text-white menu-save">Create
                                                                    menu</a>
                                                            </div>
                                                        @elseif(request()->has('menu'))
                                                            <div class="publishing-action">
                                                                <a onclick="getmenus()" name="save_menu"
                                                                    id="save_menu_header"
                                                                    class="btn btn-primary text-white menu-save">Save
                                                                    menu</a>
                                                ...
                                                <div id="post-body">
                                                    <div id="post-body-content">
                                                        @if (isset($menus) &&( request()->has('menu')|| request()->has('locale_parent_id')))
                                                                ...
                                                                @foreach (getLanguages() as $language)
                                                                    ...
                                                                    <a href="{{
                                                                        route('menus.index',[
                                                                            ...
                                                                            'locale_parent_id'=>request()->locale_parent_id ? request()->locale_parent_id : request()->menu,
                                                                            'menu'=> $lang_menus && $lang_menus->where('locale_id', $language->id)->first()?$lang_menus->where('locale_id', $language->id)->first()->id:null
                                                                        ])
                                                                    }}"
                                                                        class="text-white badge rounded-pill bg-{{ ($language->id == request()->locale_id && request()->has('locale_id')) ||(!request()->has('locale_id') && $language->id == 1)? 'success': 'primary' }} m-1">
                                                                        {{ $language->name }}
                                                                    </a>
                                                                @endforeach
                                                            </div>
