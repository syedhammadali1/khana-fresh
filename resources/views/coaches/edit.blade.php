...
@section('wrapper')
    ...
    <form class="row g-3" action="{{ route('coaches.update', ['coach' => $coach]) }}" method="POST">
        @csrf
        @method('patch')
                ...
                <div class="card">
                    <div class="card-body">
                        <div class="card-title d-flex align-items-center">
                            <h5 class="mb-0 text-primary">Edit Coach</h5>
                        </div>
                        <hr>
                        <div class="row g-2">
                            <div class="col-md-6">
                        ...
                        <div class="row">
                            <div class="col-md-12">
                                ...
                                <input type="text" class="form-control" readonly disabled
                                    value="{{ $coach->created_at->toDayDateTimeString() }}">
                            </div>
                            <div class="col-md-12">
                                <label for="inputState" class="form-label">Status*</label>
                                <select id="inputState" class="form-select  @error('status') is-invalid @enderror"
                                    name="status">
                                    <option value="publish" @if (old('status', $coach->status) == 'publish') selected @endif>Publish</option>
                                    <option value="draft" @if (old('status', $coach->status) == 'draft') selected @endif>Draft</option>
                                </select>
                                @error('status')
                        ...
                        <div class="row">
                            <div class="col-md-12">
                                ...
                                <select multiple name="coach_langauges[]"
                                    ...
                                    @foreach ($coach->coach_languages as $language)
                            ...
                            <div class="col-md-12">
                                ...
                                <select class="form-select single-select   @error('country_id') is-invalid @enderror"
                                    ...
                                    @foreach (getCountries() as $country)
                                        <option @if (old('country_id', $coach->country_id) == $country->id) selected @endif value="{{ $country->id }}">
                                            {{ $country->name }}
                                        </option>
                                    @endforeach
                                </select>
...
@push('scripts')
    ...
    <script>
        ...
        @if (count(collect(old('qualifiaction', $coach->coach_qualification))->toArray()) > 0)
            $repeater.setList(@json(old('qualifiaction', $coach->coach_qualification)));
            console.log(@json(old('qualifiaction', $coach->coach_qualification)))
        @endif
        ...
        let country_id = {{ $coach->country_id }}
        let state_id = {{ $coach->state_id }}
        let city_id = {{ $coach->city_id }}

        $.ajax({
            url: '{{ route('getStates', ['country_id' => $coach->country_id]) }}',
            type: 'get',
            success: function(res) {
                $('#state_id').html('<option value="">Select State</option>');
                $.each(res, function(key, value) {
                    let selected = state_id == value.id ? "selected" : "";
                    $('#state_id').append('<option ' + selected + ' value="' + value.id + '">' + value
                        .name + '</option>');
                });
        ...
        $.ajax({
            ...
            success: function(res) {
                ...
                $.each(res, function(key, value) {
                    ...
                    $('#city_id').append('<option ' + selected + ' value="' + value.id + '">' + value
                        .name +
                        '</option>');
                });
                jQuery('#city_id').select2({
